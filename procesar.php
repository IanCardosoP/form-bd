...
if ($conn->connect_error) {
    die(infoError("La conexión falló: " . $conn->connect_error));
}

    // Preparar y ejecutar la consulta
    $insertar = "INSERT INTO `usuarios` (`id`, `nombre`, `direccion`, `correo`, `telefono`, `estadoCivil`, `leer`, `cine`, `viajar`, `estado`) VALUES (NULL, '$nombre', '$direccion', '$correo', '$telefono', '$estado_civil', '$leer', '$cine', '$viajar', '$estado') ";
    $stmt = $conn->prepare($insertar);
    $stmt->bind_param("ssssssss", $nombre, $direccion, $correo, $telefono, $estado_civil, $leer, $cine, $viajar);
    if ($stmt->execute()) {
        echo '<script type="text/javascript">alert("Registro completado");</script>';
    } else {
        infoError( "Error al insertar datos: " . $stmt->error);
    }
